...
function getSettings(viewModel) {
	VSS.getService("ms.vss-web.data-service").then(function (extensionSettingsService) {
		...
		extensionSettingsService.getDocument("artifactoryConfig", "main", { scopeType: "Default" }).then(function (artifactoryConfig) {
			if (artifactoryConfig) {
				viewModel.artifactoryUri(artifactoryConfig.artifactoryUri);
				viewModel.userName(artifactoryConfig.credentials ? artifactoryConfig.credentials.username : "");
				viewModel.password(artifactoryConfig.credentials ? artifactoryConfig.credentials.password : "");
			...
			//Old data format (no navigation in setup Artifactory hub since new extension installed i.e no migration)
...
function saveSettings(viewModel) {
	...
	VSS.getService("ms.vss-web.data-service").then(function (extensionSettingsService) {
		extensionSettingsService.getDocument("setupBuildArtifactory", viewModel.buildDefId, { scopeType: "Default" }).then(function (loadedViewModel) {
			if (loadedViewModel) {
				loadedViewModel.username = saveViewModel.username;
				loadedViewModel.password = saveViewModel.password
				loadedViewModel.publishRepo = saveViewModel.publishRepo;
				loadedViewModel.promoteRepo = saveViewModel.promoteRepo;
				loadedViewModel.artifactoryUrl = saveViewModel.artifactoryUrl;
				loadedViewModel.overrideCreds = saveViewModel.overrideCreds;
				extensionSettingsService.setDocument("setupBuildArtifactory", loadedViewModel, { scopeType: "Default" }).then(function (value) {
					console.log(value);
				});
			}
		}, function (reason) {
